    ...
    const fetchUserExchangeProducts = async () => {
      try {
        ...
        if (userId) {
          ...
          setUserExchangeProducts(response.data.products.filter(product => product.status === "echange"));
        }
      } catch (error) {
            ...
            {userExchangeProducts.length === 0 ? (
              ...
              <div className="product-selection">
                {userExchangeProducts.map((prod) => (
                  <div
                    key={prod._id}
                    className={`product-item ${selectedProduct?._id === prod._id ? "selected" : ""}`}
                    onClick={() => setSelectedProduct(prod)}
                  >
                    <img src={`${baseURL}${prod.imageUrls[0]}`} alt={prod.nom} />
                    <h4>{prod.nom}</h4>
                  </div>
                ))}
              </div>
            )}
            <button onClick={handleExchange} className="custom-btn custom-confirm-btn">
              Submit Exchange
            </button>
            <button onClick={closeExchangeModal} className="custom-btn custom-cancel-btn">
              Cancel
            </button>
          </div>
        </div>
